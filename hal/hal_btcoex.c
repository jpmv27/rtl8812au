...
static u8 _btmpoper_cmd(PBTC_COEXIST pBtCoexist, u8 opcode, u8 opcodever, u8 *cmd, u8 size)
{
	...
	ret = rtw_hal_fill_h2c_cmd(padapter, H2C_BT_MP_OPER, buflen, buf);
	if (ret == _FAIL)
		goto exit;
...
void halbtcoutsrc_SetBtReg(void *pBtcContext, u8 RegType, u32 RegAddr, u32 Data)
{
	...
	if (halbtcoutsrc_IsHwMailboxExist(pBtCoexist) == _TRUE) {
		...
		ret = _btmpoper_cmd(pBtCoexist, BT_OP_WRITE_REG_VALUE, 0, (u8 *)&Data, 3);
		if (_FAIL == ret)
			goto out;

		buf[0] = RegType;
		*(u16 *)(buf+1) = cpu_to_le16((u16)RegAddr);
		ret = _btmpoper_cmd(pBtCoexist, BT_OP_WRITE_REG_ADDR, 0, buf, 3);
		if (_FAIL == ret)
			goto out;
...
u16 hal_btcoex_btreg_read(PADAPTER padapter, u8 type, u16 addr)
{
	...
	val = (u16)halbtcoutsrc_GetBtReg(&GLBtCoexist, type, addr);
...
{
	halbtcoutsrc_SetBtReg(&GLBtCoexist, type, addr, val);
}
